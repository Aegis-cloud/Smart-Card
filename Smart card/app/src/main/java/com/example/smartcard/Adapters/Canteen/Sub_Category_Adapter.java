...
public class Sub_Category_Adapter extends RecyclerView.Adapter<Sub_Category_Adapter.MyViewHolder> {
    List<SubCategory> category_items = new ArrayList<>();
    Context mCtx;
    ...
    public Sub_Category_Adapter(List<SubCategory> category_items, Context mCtx) {
    ...
    public MyViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        View view = LayoutInflater.from(parent.getContext()).inflate(R.layout.canteen_sub_cat_items, parent, false);
    ...
    public void onBindViewHolder(@NonNull final MyViewHolder holder, int position) {
        final int c = 1;
        final SubCategory cat_items = category_items.get(position);
        holder.cat_name.setText(cat_items.getFname());
        holder.subcat_amt.setText(cat_items.getFamount());
        holder.addproduct.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                SessionManager sessionManager = new SessionManager(mCtx);
                sessionManager.Product_Canteen(cat_items.getFid());
                taskListener.Calculation(cat_items.getFamount(),c);
                ...
                holder.minus.setOnClickListener(new View.OnClickListener() {
                    ...
                    public void onClick(View v) {
                        if (holder.count.getText().equals("1")) {
                            taskListener.Calculation(cat_items.getFamount(),0);
        ...
        Glide.with(mCtx).load(AppInterfaces.IMAGE_URL + cat_items.getFimage()).centerCrop().into(holder.cat_pic);
    }
